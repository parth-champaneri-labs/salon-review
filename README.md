...
1. Experience: select any rating, then Continue.
2. Visit details: choose one primary service and optional highlights. More services expands inline.
3. Your review: choose one of three writing suggestions or write directly, edit up to 4,000 characters, copy, and continue to Google.

Every rating follows the same steps and has the same Google destination. Back preserves selections and the draft. Changing context refreshes suggestions and clears the selected suggestion marker without replacing the draft. Explicitly selecting a suggestion replaces the textarea with that suggestion.
